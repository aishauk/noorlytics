...
def require_license_or_exit(command_name: str):
    ...
    if not license_key:
        click.echo("🔐 Set NOOR_LICENSE_KEY to run Noorlytics (free: 5 runs).")
        raise SystemExit(2)
    res = check_and_consume(license_key, product=command_name, consume=True)
    if not res.get("ok"):
        click.echo(f"❌ License check failed: {res.get('error','license_error')}")
        raise SystemExit(3)
    click.echo(f"✅ License OK ({res.get('plan')}). Remaining runs: {res.get('remaining')}")
...
def analyze_deps_cmd(state: CLIState, manifest: Path):
    ...
    if getattr(result, "notes", None):
        render_notes_cli(result.notes)
    if getattr(result, "vulnerabilities", None):
        render_vulns_cli(result.vulnerabilities)

    # Save JSON report
    out = state.reports_dir / f"{manifest.name}.deps.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(result.to_dict(), indent=2, ensure_ascii=False), encoding="utf-8")
    click.echo(click.style(f"📝  Dependency report saved → {out}", fg="green", bold=True))
